...
기획서에 머무는 아이디어가 아니라 지금 동작하는 서비스로 증명함.
...
<td width="50%"><img src="docs/img/screen-calc-installment.png" width="100%" alt="적금 계산기"><br><sub><b>Y4-1 수익률 계산기 (적금)</b> - 적립식 세후 실수령액 자동 계산</sub></td>
</tr>
<tr>
...
- **자격요건 필터링(Hard Filter)** - 수동 입력값을 상품 자격 룰과 대조해 가입 불가 상품을 제외하고, 제외 근거를 문구로 노출함
...
| 멘토·예선발표 | 독점시장 아님, 알고리즘 검증 필요 | 경쟁사 분석 진행하여 포지셔닝 수립, 4계층 검증 구조 설계 |
| 샌드버그 CTO | 추천 기능만으로는 리텐션에 한계 | 정부상품 캘린더 및 출시·만기 알림 기능 개발 |
| 중간보고회 | 저축시장 선택 이유 보강 | 금리·정책·수요 3축 확장 시장임을 리서치 기반 보강 |
...
- **상품군** - 먼저 단기예치(파킹통장·초단기 예적금, 개발 진행 중) 상품군을 추가하고, 출시 이후 주거 상품(주택청약·주거정책)으로 넓히고, 최종적으로 마이데이터를 연동해 입력 단계 없는 자동 맞춤 추천을 제공함
- **기능** - 가입 기간이 정해진 정부상품의 캘린더와 신청 일정 알림으로 재방문을 유도하고, 청년 금융의 44.4%가 오가는 커뮤니티를 정확한 정보 위에서 다루는 뉴스·커뮤니티로 확장할 예정임

### 6.2. 목표 시장 (TAM-SAM-SOM)

전체 시장은 배경으로 두고, Y-Fin.이 실제로 점유할 **틈새 시장(SOM)을 핵심 지표**로 제시함.
...
| PHASE 2 안착  | 26.10~27.06 | SOM 진입 · 누적 5천 명 | 본격 마케팅 진행, 광고 BM 도입, MAU 1,000 달성 목표 |
| PHASE 3 확장  | 27.07~29.06 | SOM 달성 · 누적 5만 명 | 마이데이터 연동, 구독 BM 및 데이터 BM 도입, MAU 5,000 달성 목표 |

### 6.4. 수익 모델

**추천 정렬에 광고비를 반영하지 않는다는 원칙** 아래, 서비스가 안착한 뒤 광고 → 구독 → 데이터 BM 순으로 수익화함. 판매·중개 수수료 모델은 금융소비자보호법상 등록 의무가 발생해, 라이선스 취득 전까지 채택하지 않음. 아래 매출은 **SOM 달성(성숙기 MAU 5만) 기준 추정**임. 예상 매출은 MAU에 정비례하며, 3년 진입기 지표는 6.5 KPI를 참고함.
