...
선택지는 정부 63개 + 은행 328개 = **총 391개**로 역대 최대인데, 이에 따라 선택의 난이도가 한층 높아짐.

- 광고 최고금리와 실제 달성 금리가 평균 1~3%p 다름
- 정책은 늘었지만 자격 판단이 어려움
- 그 결과 청년의 **64%가 비교 단계에서 이탈**, 미래적금 기여금 약 80만 명분이 미신청으로 잔존

> 지금 시장에 부족한 것은 상품이 아니라, **391개 중 "내가 받을 수 있는 것"을 가려내는 비교 도구**임. Y-Fin.의 진입 이유가 여기에 있음.

### 1.2. 개발 목표 및 주요 내용

시장조사를 통해 확인한 세 가지 핵심 문제를 해결하는 것을 목표로 함. 각 문제는 자체 설문(61명)으로 검증됨.
...
- **포용금융 / ESG 연계:** 금융위 2026 기조「생산적·포용·신뢰받는 금융」과 직접 연계됨. 다크패턴을 방지(달성 가능 금리 표시)하고 지역 청년 금융격차를 해소(영남권 우선)함.
- **신뢰 보호 원칙:** 추천 정렬에 광고비를 절대 반영하지 않고, 광고 수익은 별도 영역으로 격리·명시할 예정임.
