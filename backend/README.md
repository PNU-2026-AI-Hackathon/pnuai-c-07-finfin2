...
검색 중 조회한 상품 엔티티와 리스트가 고른 옵션을 그대로 재사용하므로 카드와 상세의 적합도·금리 값이 일치합니다. 노출하는 옵션 집합(금리표·가입기간·키워드)은 요청자가 가입 가능한 옵션 기준이라 상품별 상세 API(`/search/products/{id}/detail`, 전체 가입가능 옵션 기준)보다 좁을 수 있습니다. 비로그인 요청에서는 상세 API와 동일하게 수익 지표가 잠긴 상태로 반환됩니다.
