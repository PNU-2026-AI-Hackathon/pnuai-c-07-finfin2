...
INSERT INTO term_versions (
    term_id, major_version, minor_version, title, content, is_current, effective_from
      ...
      (
          (SELECT id FROM terms WHERE code = 'PRIVACY_STORAGE_CONSENT'),
          1, 0,
          '[선택] 개인정보 저장 및 재사용 동의 전문',
          E'**1. 목적**\n''**Y-Fin.**''은 회원이 정보 입력 화면에서 입력한 시뮬레이션 정보(생년월일, 소득, 가구 정보, 거주지역, 거래 이력, 우대조건 등)를 **저장**하여, 다음 방문 시 마이페이지에서 저장된 정보를 확인하고 재입력 없이 추천을 이어갈 수 있도록 합니다.\n\n**2. 저장 항목**\n\n- 정보 입력 화면에서 입력한 자격·추천 관련 값 일체(표시 6개 항목 및 프리필용 값 포함)\n\n**3. 보유 및 이용 기간**\n\n- **동의 시점부터 회원 탈퇴 또는 본 동의 철회 시까지** 보관합니다.\n- **동의를 철회하면 저장된 입력 정보는 지체 없이 파기됩니다.** 단, 세션 단위 시뮬레이션(추천) 이용에는 영향이 없습니다.\n\n**4. 동의 거부권 및 불이익 안내**\n\n- **본 동의는 선택 사항이며, 거부하더라도 추천 서비스 이용에는 제한이 없습니다.** 다만 미동의 시 입력 정보가 저장되지 않아 매 방문마다 정보를 다시 입력해야 합니다.\n- 동의 후에도 마이페이지에서 언제든지 철회할 수 있으며, 철회 시 저장된 정보는 삭제됩니다.',
          TRUE,
          '2026-03-12T00:00:00+09:00'
      );
