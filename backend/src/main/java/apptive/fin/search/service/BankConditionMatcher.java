...
final class BankConditionMatcher {
    ...
    // 상품에 비대면가입 우대조건이 있는지.
    // BANK_AGE와 달리 키워드 태그로만 표기된 상품이 있어 keywords/preferentialRates 양쪽을 본다.
    static boolean hasOnlineJoinCondition(ProductProperty property) {
        return property.getKeywords().stream()
                ...
                .anyMatch(keywordCode -> keywordCode == KeywordValueEnum.BANK_ONLINE_JOIN)
                || property.getPreferentialRates().stream()
                .anyMatch(rate -> rate.getKeywordCode() == KeywordValueEnum.BANK_ONLINE_JOIN);
    }
}
